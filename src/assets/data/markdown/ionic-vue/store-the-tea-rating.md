...
By laying everything out this way, we allow for change in our system without breaking a lot of code. For example, if we add an endpoint for storing user ratings of teas in the backend API. In this case, the only thing that needs to change is the `useTea` composition function to change how the `rate()` and `refresh()` access the ratings data. Nothing else in the system needs to change.
...
Two tests are using the `Tea` model when creating test data.

- `tests/unit/views/TeaList.spec.ts`
- `tests/unit/use/tea.spec.ts`

Update the `TeaList` test to include a `rating` in any defined `Tea` model. Use any number between 0 and 5 for each tea.

In the `tests/unit/use/tea.spec.ts` file, there are a few other preliminary items to handle as well:
...
- `import { Stroage } from @capacitor/storage;`
...
When the rating is saved using `@capacitor/stoage`, we will use a key of `ratingID` where ID is the value of the tea's ID. The test for this looks like the following (this should go in the "rate" `describe` block.
...
```typescript
describe('rate', () => {
  const { find, rate, refresh } = useTea();
  ...
  it('updates the tea', async () => {
    await refresh();
    await rate(5, 1);
    const tea = await find(5);
    expect(tea?.rating).toEqual(1);
  });
});
```
...
Here is the diff for the `TeaDetails` coding in case you need it.

```diff
diff --git a/src/views/TeaDetails.vue b/src/views/TeaDetails.vue
